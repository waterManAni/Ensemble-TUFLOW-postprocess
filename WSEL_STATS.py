...
def WSELpostprocessdir(resultsdir):
    clrscr()
    postprocessfolder= os.path.join (resultsdir,'_Postprocess_WSEL_'+datetime.datetime.now().strftime("%B%d"))
    try:
...
postprfolder= WSELpostprocessdir (Resultsfolder)

######GET LIST OF FILES CONTAINING THE KEYWORD########

listofresults = findfiles (Resultsfolder,['_h_Max.flt'],[''])
classifiedresults=classify_and_sort_files(listofresults, tcffile, '_h_Max')
classifiedresults.to_csv (os.path.join (postprfolder, 'WSELgrids.csv'), index=False)
